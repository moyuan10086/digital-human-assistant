...
[📥 点击下载演示视频 (20MB)](./演示.mp4)

> 演示视频展示了数字人的实时对话功能，包括文字聊天、语音识别和会话管理等核心特性。
>
> 💡 **提示**: 视频使用 Git LFS 存储，克隆仓库后可直接查看。
...
- **🛡️ 内容安全过滤**：Input/Output 双重过滤机制，确保对话内容安全
  - **输入端过滤**：关键词检测，拦截用户违规输入
  - **输出端过滤**：System Prompt 约束 + 关键词检测，防止 AI 生成违规内容
  - **Sensitive-lexicon 集成**：支持 70,000+ 敏感词库，覆盖政治、色情、暴力等领域
  - **违规日志记录**：自动记录所有违规尝试，便于审计和优化
...
│   ├── content_filter.py      # 内容安全过滤核心模块
│   ├── sensitive_lexicon_loader.py  # Sensitive-lexicon 词库加载器
│   ├── Sensitive-lexicon/     # 敏感词库 (需克隆，70,000+ 关键词)
...
系统已内置基础敏感词库（250+ 关键词）。如需更强大的过滤能力，可克隆 Sensitive-lexicon 词库：
...
> 详细配置说明请参考 [backend/CONTENT_FILTER.md](backend/CONTENT_FILTER.md)
